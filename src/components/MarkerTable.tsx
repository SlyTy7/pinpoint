...
function descendingComparator<T>(a: T, b: T, orderBy: keyof T | "coords") {
  ...
  return (b[orderBy] as any).toString().localeCompare((a[orderBy] as any).toString());
}
...
function EnhancedTableHead(props: {
  ...
  numSelected: number;
  rowCount: number;
  onRequestSort: (event: React.MouseEvent<unknown>, property: keyof MarkerData | "coords") => void;
}) {
  const { onSelectAllClick, order, orderBy, numSelected, rowCount, onRequestSort } = props;
  const createSortHandler = (property: keyof MarkerData | "coords") => (event: React.MouseEvent<unknown>) =>
...
export default function MarkerTable({
  ...
  const [orderBy, setOrderBy] = React.useState<keyof MarkerData | "coords">("id");
  ...
  const handleClick = (event: React.MouseEvent<unknown>, id: number, coords: [number, number]) => {
            ...
            <TableBody>
              {visibleRows.map((row) => {
                ...
                return (
                  <TableRow
                    hover
                    onClick={(event) => handleClick(event, row.id, row.coords)}
                    role="checkbox"
                  ...
                  >
                    ...
                    <TableCell>{row.coords.join(", ")}</TableCell>
                  </TableRow>
                );
              })}
